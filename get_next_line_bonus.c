...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/01/28 17:37:36 by rsaid             #+#    #+#             */
/*   Updated: 2023/01/29 21:02:41 by rsaid            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	{
		storage[fd] = ft_strjoin(storage[fd], vars.buffer);
		vars.read_size = read(fd, vars.buffer, BUFFER_SIZE);
		vars.buffer[vars.read_size] = '\0';
	}
	if (vars.read_size == 0)
		vars.line = linemaker(storage[fd], '\0');
	if (storage[fd] != NULL)
		vars.buffer = ft_strjoin_with_free(storage[fd], vars.buffer);
	storage[fd] = ft_strchr(vars.buffer, '\n');
	if (vars.read_size > 0)
